...
&emsp;では、まずはなんの工夫もなしに大量のオブジェクトを描画してみましょう。ただし、このハンズオンは特段変わったことはしていません。後に実装する4.2「インスタンシング描画」と4.3「モデルをオフスクリーン描画して、板ポリをインスタンシング描画」との対比のために実装をするだけです。ではAppendix_04_01/Appendix_04_01/slnを開いてください。
#### step-1 人物のモデルを初期化
...
&emsp;4.1.1と同様のモデルを、今度はインスタンシング描画で500体表示してみます。4.1.1のプログラムとの差異を意識して実装してきましょう。では、Appendix_04_02/Appendix_04_02.slnを立ち上げてください。
...
&emsp;では、早速実装していきましょう。今回実装する必要があるのはc++側だけです。今回は4.2のインスタンシング描画から変更を行う必要がある点だけに着目して実装していきます。Appendix_04_03/Appendix_04_03.slnを開いてください。
